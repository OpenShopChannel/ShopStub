...
int extractZip(unzFile uf, int opt_extract_without_path, int opt_overwrite,
               const char *password) {
    zip_progress = 0;
    zip_size = zipInfo(uf);
    ...
    for (i = 0; i < gi.number_entry; i++) {
        ...
        // gi.number_entry, zip_progress, zip_size);
        printf(".");
...
int extractZipOnefile(unzFile uf, const char *filename,
