...
zlib_filefunc_def *pzlib_filefunc_def;
{
    ...
    us.filestream =
        (*(us.z_filefunc.zopen_file))(us.z_filefunc.opaque, path,
...
*/
extern int ZEXPORT unzLocateFile(file, szFileName,
                                 iCaseSensitivity) unzFile file;
...
local int unzlocal_CheckCurrentFileCoherencyHeader(
    s, piSizeVar, poffset_local_extrafield, psize_local_extrafield) unz_s *s;
uInt *piSizeVar;
...
*/
extern int ZEXPORT unzOpenCurrentFile3(file, method, level, raw,
                                       password) unzFile file;
int *method;
int *level;
int raw;
const char *password;
